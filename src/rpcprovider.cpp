...
void RpcProvider::onMessage(
    ...
    buf.copy(reinterpret_cast<char*>(&header_size), 4, 0);
    ...
    if( rpcheader.ParseFromString(buf.substr(4, header_size)) ) {// 从第4字节开始, 拷贝头部
        service_name = rpcheader.service_name();
        method_name = rpcheader.method_name();
        args_size = rpcheader.args_size();

        args_str = buf.substr(4 + header_size, args_size);
...
void RpcProvider::MyClosure::Run() {
    // nothing
}